...
func _ready():
	call_deferred("_state_default_state") # Called when all nodes, state machine included, are ready
...
#region Auto-called methods
# These methods will call the supposed auto-called methods of each node
# Methods of each node are named slightly differently to allow calling only those we want
